...
{
    char *endptr;
    int num = strtol(s.c_str(), &endptr, 0);
    return num;
}
   ...
   if(!asm_file.is_open())
   {
    ...
    while(getline(asm_file,line))
    {
        ...
        else
        {
            ...
            else if(type == 3)
            {
                ...
                {
                    label_add[label] = strtoint(token);
                }
            }   
            else
            {
    ...
    while(getline(asm_file,line))
    {
        ...
        if(type > 0)
        {
            tokens >> opr;
            // cout<<opr<<endl;
            if(memory.find(opr)!= memory.end() && type ==1)
            {
                binary= bitset<24>(memory[opr]).to_string() + binary;
            }
            else if(label_add.find(opr)!=label_add.end())
            {
                // cout<<label_add[opr];
                if( type == 4 )
            ...
            {
                binary = bitset<24>(strtoint(opr)).to_string() + binary;
            }
        }
        else
        {
            binary = bitset<24>(0).to_string() + binary;
        }
        // cout<<binary;
        asm_code<< bintohex(binary) << endl;
        list_file<< bintohex(bitset<32>(PC).to_string()) << " " << bintohex(binary) << " " << line1 << endl;
        PC++;
